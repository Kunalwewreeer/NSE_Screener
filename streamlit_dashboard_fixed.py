#!/usr/bin/env python3
"""
🚀 STREAMLIT COMPREHENSIVE INTRADAY TRADING DASHBOARD - FIXED VERSION
====================================================================
...
<style>
    ...
    .signal-neutral { background-color: #fff3cd; color: #856404; padding: 0.25rem 0.5rem; border-radius: 0.25rem; font-weight: bold; }
</style>
...
def main():
    ...
    if 'stock_data' in st.session_state:
        ...
        start_date = st.session_state.get('start_date', analysis_date - timedelta(days=3))
        ...
        st.dataframe(display_df[['Symbol', 'Price', 'Change %', 'Signal', 'Strength', 'RSI', 'Volume']], use_container_width=True)
            ...
            if chart_mode == "Single Day":
                ...
                if selected_stock_full in all_data:
                    ...
                    if not sample_data.empty:
                        try:
                            # Fix: Convert to Series first, then get unique dates
                            dates_series = pd.Series(sample_data.index.date)
                            available_dates = sorted(dates_series.unique(), reverse=True)
                        except Exception:
                            # Fallback: manual extraction
                            available_dates = sorted(list(set(sample_data.index.date)), reverse=True)
        ...
        if selected_stock in all_data:
            ...
            if fig and metrics:
                ...
                col6.metric("MACD", f"{metrics['macd']:.2f}")
